...
def crc(inBytes, init=0x00000000):
  return crcmod.mkCrcFun(0x104C11DB7, initCrc=init, rev=True, xorOut=0x00000000)(inBytes)

def undoAppendZero(currentCRC):
  for x in crcTable:
    if x >> 24 == c2 >> 24:
      i = crcTable.index(x)
      cUndo = i + (0xffffff00 & (c2 ^ x) << 8 )
      return(cUndo)
